...
void PillarBasedRemoval::removal_stage() {
    ...
    */
    ...
    auto hi_rw = pillars_highest_.data_ptr<float>();
    auto lo_rw = pillars_lowest_.data_ptr<float>();
    auto diff_rw = diff.data_ptr<float>();

    // for(size_t i = 0; i < pillars_highest_.dim(0); i++) {
    //     diff_rw[(int)i] = hi_rw[(int)i] - lo_rw[(int)i];
    // }
